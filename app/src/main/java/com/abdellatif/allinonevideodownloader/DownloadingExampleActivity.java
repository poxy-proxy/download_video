...
public class DownloadingExampleActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Скачивание видео по url");

        initViews();
        Bundle arguments = getIntent().getExtras();
        if(arguments!=null) {
            String UrlVideo = arguments.get("UrlVideo").toString();
            if (UrlVideo != null && UrlVideo != "") {
                etUrl.setText(UrlVideo);
            }
        }
        initListeners();
    }
       ...
       // }
        ...
        Disposable disposable = Observable.fromCallable(() -> YoutubeDL.getInstance().execute(request, (String) null,  callback))
                ...
                .subscribe(youtubeDLResponse -> {
                    ...
                    Toasty.success(DownloadingExampleActivity.this, "Загрузка успешно завершена", Toast.LENGTH_LONG).show();
                    
                    downloading = false;
                }, e -> {
                    if (BuildConfig.DEBUG) Log.e(TAG, "ошибка при загрузке", e);
                    pbLoading.setVisibility(View.GONE);
                    ...
                    Toasty.error(DownloadingExampleActivity.this, "Ошибка при загрузке", Toast.LENGTH_LONG).show();
                    downloading = false;
                });
